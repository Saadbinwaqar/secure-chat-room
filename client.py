...
class ChatWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self):
        ...
        self.client_names = {}         # addr_str -> username
        self.running = True
        ...
        while self.running:
            try:
                ...
                if "keys_update" in obj:
                    self.network_signal.emit("keys_update", {"keys": obj["keys_update"], "usernames": obj.get("usernames", {})})
                    continue
        ...
        if kind == "keys_update":
            ...
            for addr, pem in keys.items():
                ...
                try:
                    ...
                    username = usernames.get(addr, f"User {len(self.clients_public_keys)}")
                    self.client_names[addr] = username
                    self.contacts_list.addItem(f"{username} — {addr}")
                except Exception:
                    continue
            self.add_bubble("[Users updated]", me=False)
            return
